...
/// </summary>
public class GeospatialLocationManager2 : MonoBehaviour
{
    public GameObject distanceparent;
    public Text txtDistance;
    public GameObject AcuracyNotGood;
    ...
    private IEnumerator LoadLocationsFromRemote()
    {
        ...
        {
            request.timeout = 8; // ثانیه؛ اگه سرور تا ۸ ثانیه جواب نداد، به‌جای معطل موندن، fail میشه و پیش‌فرض‌ها استفاده میشن
    ...
    void Update()
    {
        AcuracyNotGood.SetActive(false);
    ...
    private void UpdateDistancesUI(double lat, double lon)
    {
        ...
        if (currentActiveIndex >= 0)
        {
            var activeLoc = locations[currentActiveIndex];

            distanceText.text = anchorResolveInProgress
                ? "Look at the tower's location\n(Resolving anchor...)"
                : $"Look at the tower's location\nRotation: {activeLoc.fixedYRotation:F0}\xB0";



            txtDistance.text = "VANTAGE POINT "+(currentActiveIndex+1);
        ...
        if (nearestIndex >= 0)
        {
            // Index از صفر شروع میشه، پس برای نمایش به کاربر +1 می‌کنیم
            int pointNumber = nearestIndex + 1;

            distanceText.text = $"Walk {nearestDistance:F0} meters to point {pointNumber}";
            txtDistance.text = $"Walk {nearestDistance:F0} meters to point {pointNumber}";
    ...
    private void CheckTriggers(double lat, double lon)
    {
        ...
        for (int i = 0; i < locations.Count; i++)
        {
            ...
            if (d <= locations[i].triggerDistanceMeters)
            {
                matchIndex = i;
                break;
            }
        ...
        try
        {
            ...
            else
            {
                ...
                if (result.TerrainAnchorState == TerrainAnchorState.Success && result.Anchor != null)
                {
                    ...
                    loc.spawnedInstance.transform.localRotation = Quaternion.Euler(0f, loc.fixedYRotation, 0f);
    ...
    /// <summary>
    /// این متد رو به OnClick یه دکمه‌ی "بچرخون +" وصل کن.
    /// فقط وقتی یه مدل الان spawn شده کار می‌کنه؛ زاویه‌ی fixedYRotation همون
    /// لوکیشن رو تغییر میده و بلافاصله روی مدل زنده اعمالش می‌کنه، تا بشه
    /// توی خود محیط چشمی تنظیمش کرد. مقدار نهایی رو بعداً دستی توی
    /// Inspector برای همون لوکیشن ذخیره کن تا دفعه‌ی بعد پیش‌فرض بشه.
    /// </summary>
    public void RotateActiveModel(float deltaDegrees)
    {
        if (currentActiveIndex < 0 || currentActiveIndex >= locations.Count)
        {
            Debug.LogWarning("[Geospatial] هیچ مدلی الان فعال نیست که بشه چرخوندش.");
            return;
        }

        var loc = locations[currentActiveIndex];
        loc.fixedYRotation += deltaDegrees;
        ...
        Debug.Log($"[Geospatial] {loc.label} fixedYRotation is now {loc.fixedYRotation}");
    }
}
